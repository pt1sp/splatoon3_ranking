...
if ($tableName == "all") {
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT 名前,id,武器,パワー,'バニラ派' AS 陣営 FROM ice_07_01
    ...
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT * FROM ( SELECT 名前,id,武器,パワー,'バニラ派' AS 陣営 FROM ice_07_01
    UNION ALL SELECT 名前,id,武器,パワー,'ストロベリー派' AS 陣営 FROM ice_07_02
    UNION ALL SELECT 名前,id,武器,パワー,'チョコミント派' AS 陣営 FROM ice_07_03 ) AS aaa 
