...
if ($tableName == "all") {
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT 名前,id,武器,パワー,'力派' AS 陣営 FROM zelda_06_01
    ...
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT * FROM ( SELECT 名前,id,武器,パワー,'力派' AS 陣営 FROM zelda_06_01
    UNION ALL SELECT 名前,id,武器,パワー,'知恵派' AS 陣営 FROM zelda_06_02
    UNION ALL SELECT 名前,id,武器,パワー,'勇気派' AS 陣営 FROM zelda_06_03 ) AS aaa 
