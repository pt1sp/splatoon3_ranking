...
if ($tableName == "all") {
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT 名前,id,武器,パワー,'仲間とわいわい派' AS 陣営 FROM holiday_12_01
    ...
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT * FROM ( SELECT 名前,id,武器,パワー,'仲間とわいわい派' AS 陣営 FROM holiday_12_01
    UNION ALL SELECT 名前,id,武器,パワー,'家族でほっこり派' AS 陣営 FROM holiday_12_02
