...
if ($tableName == "all") {
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT 名前,id,武器,パワー,'フウカ派' AS 陣営 FROM reader_09_01
    ...
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT * FROM ( SELECT 名前,id,武器,パワー,'フウカ派' AS 陣営 FROM reader_09_01
    UNION ALL SELECT 名前,id,武器,パワー,'ウツホ派' AS 陣営 FROM reader_09_02
    UNION ALL SELECT 名前,id,武器,パワー,'マンタロー派' AS 陣営 FROM reader_09_03 ) AS aaa 
