...
if ($tableName == "all") {
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT 名前,id,武器,パワー,'辛い派' AS 陣営 FROM taste_03_01
    ...
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT * FROM ( SELECT 名前,id,武器,パワー,'辛い派' AS 陣営 FROM taste_03_01
    UNION ALL SELECT 名前,id,武器,パワー,'甘い派' AS 陣営 FROM taste_03_02 
    UNION ALL SELECT 名前,id,武器,パワー,'すっぱい派' AS 陣営 FROM taste_03_03 
