...
if ($tableName == "all") {
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT 名前,id,武器,パワー,'富派' AS 陣営 FROM life_08_01
    ...
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT * FROM ( SELECT 名前,id,武器,パワー,'富派' AS 陣営 FROM life_08_01
    UNION ALL SELECT 名前,id,武器,パワー,'名声派' AS 陣営 FROM life_08_02
    UNION ALL SELECT 名前,id,武器,パワー,'愛派' AS 陣営 FROM life_08_03 ) AS aaa 
