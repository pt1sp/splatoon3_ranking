...
if ($tableName == "all") {
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT 名前,id,武器,パワー,'ビター派' AS 陣営 FROM choco_04_01
    ...
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT * FROM ( SELECT 名前,id,武器,パワー,'ビター派' AS 陣営 FROM choco_04_01
    UNION ALL SELECT 名前,id,武器,パワー,'ミルク派' AS 陣営 FROM choco_04_02
    UNION ALL SELECT 名前,id,武器,パワー,'ホワイト派' AS 陣営 FROM choco_04_03 ) AS aaa 
