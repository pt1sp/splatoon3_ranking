...
if ($tableName == "all") {
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT 名前,id,武器,パワー,'回転焼き派' AS 陣営 FROM name_11_01
    ...
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT * FROM ( SELECT 名前,id,武器,パワー,'回転焼き派' AS 陣営 FROM name_11_01
    UNION ALL SELECT 名前,id,武器,パワー,'大判焼き派' AS 陣営 FROM name_11_02
    UNION ALL SELECT 名前,id,武器,パワー,'今川焼き派' AS 陣営 FROM name_11_03 ) AS aaa 
