...
if ($tableName == "all") {
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT 名前,id,武器,パワー,'くさ派' AS 陣営 FROM partner_02_01
    ...
    $sql = "SELECT *, ROW_NUMBER() OVER () AS 順位 FROM ( SELECT * FROM (  SELECT 名前,id,武器,パワー,'くさ派' AS 陣営 FROM partner_02_01
    UNION ALL SELECT 名前,id,武器,パワー,'ほのお派' AS 陣営 FROM partner_02_02 
    UNION ALL SELECT 名前,id,武器,パワー,'みず派' AS 陣営 FROM partner_02_03 ) AS aaa
